...
# Load the dataset
association_2017_2020_df = pd.read_csv('./data/output.csv') # This was the dataset from the jupyter notebook where I did EDA and data transformation
...
# Categorize distance to MRT into bins: Very Close, Close, Moderate
...
# Dropping unnecessary columns for association rule mining I keep only the categorical data needed for the FP-Growth algorithm
...
if st.button('Submit'):
    ...
    # Start with the full dataset and filter based on user input, this commands actually copies from the dataset that was read initially
...
# Loop through each key-value pair in the user_input dictionary
    ...
    # Display recommendations section: based on the filtered data --------------------------------
    if not filtered_df.empty: # If matching rows are found
        st.write('Here are some HDB flats that match your criteria:')
        for index, row in filtered_df.iterrows(): #going through all matching rows and display the following fields
    ...
    # # Recommend based on the rules
    # if not matching_rules.empty:
    #     st.write('Recommendation based on your input:')
